// GET /api/ai/graph/:user_id/subject/:subject — 单学科知识图谱
// 对应后端：backend/routes/ai-graph.js GET /:user_id/subject/:subject
...
export async function onRequestGet({ params, env }) {
    try {
        const { user_id, subject } = params;

        const allKp = (await listTable(env, 'knowledge_points')).filter(k => k.subject === subject);
        ...
        return Response.json({
            code: 0,
            data: { user_id, subject, summary, nodes, edges }
        });
    } catch (e) {
        return Response.json({ code: 1, msg: '加载学科图谱失败: ' + e.message }, { status: 500 });
    }
}
