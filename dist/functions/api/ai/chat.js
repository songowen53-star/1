...
// ========== RAG 知识库（Cloudflare 边缘版，内联） ==========
// 注意：Cloudflare Pages Functions 为边缘运行时，无法直接 require backend/rag/kb-store.js
// 此处保留内联副本，数据源与 backend/data/ai-kb.json 保持同步
// 后端主实现已迁移至 rag/kb-store.js + data/ai-kb.json（支持向量检索扩展）
const AI_KB = [
...
// ========== 算术求解器（安全版，无 eval/Function，递归下降解析） ==========
// v1.1 已弃用 Function 构造函数，改用手写 tokenizer + 递归下降解析器
// 支持 + - * / ^ ( ) 与小数，严格白名单，杜绝代码注入
...
function parseExpr(tokens, pos) {
    let left, np;
    ({ val: left, pos: np } = parseTerm(tokens, pos));
    while (np < tokens.length && tokens[np].type === 'op' && (tokens[np].value === '+' || tokens[np].value === '-')) {
        const op = tokens[np].value; np++;
        let right;
        ({ val: right, pos: np } = parseTerm(tokens, np));
        left = op === '+' ? left + right : left - right;
    }
    return { val: left, pos: np };
}

function parseTerm(tokens, pos) {
    let left, np;
    ({ val: left, pos: np } = parsePow(tokens, pos));
    while (np < tokens.length && tokens[np].type === 'op' && (tokens[np].value === '*' || tokens[np].value === '/')) {
        const op = tokens[np].value; np++;
        let right;
        ({ val: right, pos: np } = parsePow(tokens, np));
        if (op === '*') left = left * right;
        else { if (right === 0) return { val: NaN, pos: np }; left = left / right; }
    }
    return { val: left, pos: np };
}
...
function parsePow(tokens, pos) {
    let left, np;
    ({ val: left, pos: np } = parseFactor(tokens, pos));
    while (np < tokens.length && tokens[np].type === 'op' && tokens[np].value === '^') {
        np++;
        let right;
        ({ val: right, pos: np } = parsePow(tokens, np)); // 右结合
...
function parseFactor(tokens, pos) {
    ...
    if (t.type === 'op' && t.value === '(') {
        ...
        let np = r.pos;
        if (np < tokens.length && tokens[np].type === 'op' && tokens[np].value === ')') np++;
        return { val: r.val, pos: np };
    }
    if (t.type === 'op' && t.value === '-') {
    ...
    if (t.type === 'num') return { val: t.value, pos: pos + 1 };
    return { val: NaN, pos };
}
...
function safeEvalArith(expr) {
    ...
    if (r.pos !== tokens.length) return null; // 未完全消费=语法错误
    return (typeof r.val === 'number' && !isNaN(r.val) && isFinite(r.val)) ? r.val : null;
}
...
function solveArithmetic(question) {
    ...
    const result = safeEvalArith(expr);
