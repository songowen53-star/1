...
window.__PHOTO_CAMERA__ = {
    ...
    shoot: function (afterTarget) {
        var self = this;
        // 生成模拟原题图片（数学题试卷样式 SVG）
        var mockQ = [
            { t: '数学 · 导数与函数单调性', q: '已知函数 f(x) = ln x − ax（a ∈ R）。', s1: '（1）讨论 f(x) 的单调性；', s2: '（2）若 f(x) ≤ 0 恒成立，求 a 的取值范围。' },
            { t: '数学 · 三角函数', q: '已知函数 f(x) = 2sin(2x + π/6)。', s1: '（1）求 f(x) 的最小正周期；', s2: '（2）求 f(x) 在区间 [0, π/2] 上的最大值。' },
            { t: '物理 · 电磁感应', q: '如图所示，导体棒 ab 在匀强磁场中运动。', s1: '（1）判断感应电流方向；', s2: '（2）求感应电动势大小。' },
            { t: '化学 · 有机化学', q: '已知有机物 A 的分子式为 C₄H₈O₂。', s1: '（1）若 A 能发生银镜反应，写出结构简式；', s2: '（2）若 A 能与 NaHCO₃ 反应，写出结构简式。' }
        ];
        var q = mockQ[Math.floor(Math.random() * mockQ.length)];
        var svg = '<svg xmlns="http://www.w3.org/2000/svg" width="400" height="260" viewBox="0 0 400 260"><rect width="400" height="260" fill="#ffffff"/><rect x="0" y="0" width="400" height="34" fill="#EFF6FF"/><text x="14" y="23" font-family="serif" font-size="14" font-weight="700" fill="#1E40AF">' + q.t + '</text><text x="14" y="66" font-family="serif" font-size="15" fill="#111827">' + q.q + '</text><text x="14" y="96" font-family="serif" font-size="15" fill="#111827">' + q.s1 + '</text><text x="14" y="126" font-family="serif" font-size="15" fill="#111827">' + q.s2 + '</text><line x1="14" y1="150" x2="386" y2="150" stroke="#E5E7EB" stroke-width="1"/><text x="14" y="178" font-family="serif" font-size="13" fill="#6B7280">【解】</text><text x="40" y="202" font-family="serif" font-size="13" fill="#374151">由题意可得 ……</text><text x="40" y="226" font-family="serif" font-size="13" fill="#374151">（解题过程省略）</text></svg>';
        window.__MOCK_QUESTION_IMAGE__ = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
        ...
        setTimeout(function () {
            ...
            var next = function () {
                if (i >= steps.length) {
                    // 保存模拟原题图片（拍照/相册选择后传递给解析页）
                    window.__LAST_PHOTO_IMAGE__ = window.__MOCK_QUESTION_IMAGE__ || null;
                    self.close();
...
registerPage('photo-parse', 'AI解析', '拍照搜题', 'fa-magic', function () {
    ...
    function renderPageContent(container, data) {
        ...
        // 原题图片：优先使用拍照时保存的图片，其次使用 mock 数据中的 original_image
        var origImg = (window.__LAST_PHOTO_IMAGE__) || data.original_image;
        if (origImg) {
            html += `
            <div style="background:#F9FAFB;border-radius:12px;padding:10px;margin-bottom:12px;border:1px solid var(--border);">
                <div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;font-weight:600;"><i class="fas fa-image" style="color:var(--primary);margin-right:4px;"></i>原题图片</div>
                <img src="${origImg}" alt="原题图片" style="width:100%;border-radius:8px;display:block;" onerror="this.outerHTML='<div style=\\'text-align:center;padding:24px;color:var(--text-tertiary);\\'><i class=\\'fas fa-image\\' style=\\'font-size:28px;\\'></i><div style=\\'font-size:12px;margin-top:6px;\\'>原题图片加载失败</div></div>'">
            </div>`;
        } else {
            html += `
