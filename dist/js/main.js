...
function initInteractions() {
    ...
    if (chatInput && !chatInput._imeBound) {
        chatInput._imeBound = true; // 避免重复绑定
        ...
        container.innerHTML = `
            ...
            <div class="row">
                <div class="card" style="flex:1;max-width:480px;margin:0 auto;">
                    ...
                    <!-- 模块1：密码登录/注册 -->
                    <div id="tab-password" class="auth-tab-panel">
                    ...
                    <!-- 模块2：微信登录（一键授权 + 扫码登录） -->
                    <div id="tab-wechat" class="auth-tab-panel" style="display:none;">
                            ...
                            <div id="wx-qrcode-box" style="text-align:center;">
                                <div id="wx-qrcode-wrap" style="position:relative;display:inline-block;margin:4px auto 14px;padding:16px;background:white;border:1px solid #E5E7EB;border-radius:16px;">
                                    ...
                                    <div id="wx-qrcode-mask" style="display:none;position:absolute;inset:16px;background:rgba(255,255,255,0.92);border-radius:12px;align-items:center;justify-content:center;flex-direction:column;padding:12px;">
                                        <div id="wx-mask-icon" style="font-size:44px;color:#07C160;margin-bottom:10px;"><i class="fas fa-mobile-alt"></i></div>
                                        <div id="wx-mask-text" style="font-size:14px;font-weight:600;color:#111827;">等待扫码...</div>
                                    </div>
                                </div>
                                <div id="wx-qrcode-tip" style="font-size:14px;font-weight:600;color:#111827;margin-bottom:6px;">请使用微信扫一扫登录</div>
                                <div id="wx-qrcode-subtip" style="font-size:12px;color:#6B7280;margin-bottom:14px;">二维码 <span id="wx-qrcode-countdown">300</span>s 后自动刷新，<a href="javascript:void(0)" onclick="refreshWechatQrcode()" style="color:#07C160;text-decoration:none;">点此刷新</a></div>
                                <!-- 演示模式操作：模拟扫码 -->
                                <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;padding:12px;text-align:left;">
                                    <div style="font-size:12px;font-weight:700;color:#166534;margin-bottom:8px;"><i class="fas fa-vial"></i> 演示模式（无真实微信环境时使用）</div>
                                    <div style="display:flex;gap:8px;flex-wrap:wrap;">
                                        <button class="btn-primary" style="background:#07C160;border-radius:10px;height:36px;font-size:13px;padding:0 12px;flex:1 1 auto;" onclick="mockWechatAction('scan')"><i class="fas fa-mobile-alt"></i> 模拟扫码</button>
                                        <button class="btn-primary" style="background:#16A34A;border-radius:10px;height:36px;font-size:13px;padding:0 12px;flex:1 1 auto;" onclick="mockWechatAction('confirm')"><i class="fas fa-check-circle"></i> 模拟确认</button>
                                        <button type="button" style="background:white;border:1px solid #E5E7EB;color:#6B7280;border-radius:10px;height:36px;font-size:13px;padding:0 12px;flex:1 1 auto;cursor:pointer;" onclick="mockWechatAction('cancel')"><i class="fas fa-times-circle"></i> 模拟取消</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- 模块3：手机验证码登录 -->
                    <div id="tab-sms" class="auth-tab-panel" style="display:none;">
                    ...
                    <!-- 模块4：家长绑定 -->
                    <div id="tab-parent" class="auth-tab-panel" style="display:none;">
                        ...
                        <form id="parent-bind-form" class="auth-form" onsubmit="handleParentBind(event)">
                            ...
                            <div class="auth-field">
                                <label>与学生关系</label>
                                <div style="display:flex;gap:10px;margin-top:6px;" id="relation-group">
                            ...
                            <div class="auth-field">
                                ...
                                <div style="display:flex;gap:10px;">
                                    ...
                                    <button type="button" onclick="handleSmsSendForParent()" style="flex-shrink:0;width:130px;background:white;border:1.5px solid #3B82F6;color:#3B82F6;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;">获取验证码</button>
                                </div>
                            </div>
...
function switchAuthSubTab(tab, subTab) {
    ...
    document.getElementById('login-form').style.display = subTab === 'login' ? 'block' : 'none';
    document.getElementById('register-form').style.display = subTab === 'register' ? 'block' : 'none';
}
...
function switchWechatSubTab(sub) {
    ...
    const oauth = document.getElementById('wx-sub-oauth');
    const qrcode = document.getElementById('wx-sub-qrcode');
    if (oauth) oauth.style.display = sub === 'oauth' ? 'block' : 'none';
    if (qrcode) qrcode.style.display = sub === 'qrcode' ? 'block' : 'none';
    if (sub === 'qrcode') {
        refreshWechatQrcode();
    } else {
...
async function handleSmsSend() {
    ...
    if (data && data.code) {
        ...
        `);
        // 60秒倒计时
        startSmsCountdown('sms-send-btn');
...
// 家长绑定专用验证码发送
async function handleSmsSendForParent() {
    const phone = document.getElementById('bind-parent-phone').value.trim();
    if (!phone || !/^1\d{10}$/.test(phone)) {
...
async function handleParentBind(event) {
    event.preventDefault();
    const studentPhone = document.getElementById('bind-student-phone').value.trim();
    const parentPhone = document.getElementById('bind-parent-phone').value.trim();
    const relation = window._selectedRelation || '父亲';
    const smsCode = document.getElementById('bind-sms-code').value.trim();
...
async function handleRegister(event) {
    ...
    const agreeBox = document.querySelector('#register-form .agree-box');
    if (!username || !password) return showToast('请输入用户名和密码', 'warning');
