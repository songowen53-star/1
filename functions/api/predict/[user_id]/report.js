// GET /api/predict/:user_id/report — 模考预测完整报告（首页 AI今日提分：预测分数/排名/院校概率/薄弱考点建议）
import { listTable } from '../../../_shared/kv-db.js';

// 科目满分配置（数学/语文/英语/物理/化学 150，生物/政治/历史/地理 100）
const SUBJECT_MAX = {
    '数学': 150, '语文': 150, '英语': 150,
    '物理': 150, '化学': 150,
    '生物': 100, '政治': 100, '历史': 100, '地理': 100
};
// 核心预测科目（仅用于 currentPredicted 计算，与后端保持一致）
...
export async function onRequestGet({ params, env }) {
    try {
        ...
        const user = users.find(u => u.id === userId) || users[0] || {
            id: userId, province: '四川', current_score: 580, target_score: 650, exam_date: '2027-06-07'
        };

        const kpList = await listTable(env, 'knowledge_points');
        const answerRecords = await listTable(env, 'answer_records');

        // 雷达图展示全部 9 科，预测分仍按 5 核心科计算（与后端一致）
        ...
        const examDate = new Date(user.exam_date || '2027-06-07');
        ...
        const weakAdvice = kpList
            .filter(k => k.mastery_rate < 0.6)
            .sort((a, b) => (b.score_gain || 0) - (a.score_gain || 0))
            .slice(0, 3)
            .map(k => `加强「${k.name}」练习，掌握率${Math.round((k.mastery_rate || 0) * 100)}%，预计可提${k.score_gain}分`);
        ...
        return Response.json({
            ...
            data: {
                ...
                probability: {
                    yiben: Math.round(sigmoid((finalScore - 520) / 15) * 100),
                    p211,
                    p985
                },
        ...
        return Response.json({ code: 1, msg: '加载预测报告失败: ' + e.message }, { status: 500 });
    }
}
